<?php
// imprimir_dia.php - Generar PDF con facturas del día
...
$pdf->SetTitle('Facturas del Día - ' . date('d/m/Y'));
$pdf->SetSubject('Facturas del Día');
...
$pdf->Cell(0, 10, 'Facturas del Día - ' . date('d/m/Y'), 0, 1, 'C');

$pdf->Ln(10);

if (empty($pedidos)) {
    $pdf->SetFont('helvetica', '', 12);
    $pdf->Cell(0, 10, 'No hay pedidos para el día de hoy.', 0, 1, 'C');
} else {
    // Cabecera de tabla
    ...
    $pdf->Cell(0, 10, 'Total del Día: $' . number_format($totalDia, 2), 0, 1, 'R');
}

$pdf->Output('facturas_dia_' . date('Y-m-d') . '.pdf', 'I');
?>